...
int main(void)
{
	...
	xil_printf("\r\n");
	xil_printf("Is the camera sensor connected? (Y => press SW19)\r\n");
	u32 InputData;
	Status = GpioPolledExample(XPAR_PSU_GPIO_0_BASEADDR, &InputData);
	if (Status != XST_SUCCESS) {
		printf("GPIO Polled Mode Example Test Failed\r\n");
		return XST_FAILURE;
	}
	printf("Data read from GPIO Input is  0x%x \n\r", (int)InputData); //Press SW19 to replace camera sensor yes
