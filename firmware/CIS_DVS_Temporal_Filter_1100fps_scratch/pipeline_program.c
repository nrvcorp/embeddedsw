...
u32 dvs_wr_ptr = 0;
u32 dvs_frm_cnt = 0; // only for culc fps
...
void DmaWriteDoneCallback (XAxiDma_BdRing * RxRingPtr) {
	...
	//Xil_Out8(dvs_frame_rdy[ dvs_wr_ptr % DVS_BUFFER_NUM ], 0x1);
...
int DVSDma_WriteSetup(XAxiDma * AxiDmaInstPtr) {
	...
	BdCount = XAxiDma_BdRingCntCalc(XAXIDMA_BD_MINIMUM_ALIGNMENT,
				DVS_RX_BD_SPACE_HIGH - DVS_RX_BD_SPACE_BASE + 1);


	Status = XAxiDma_BdRingCreate(RxRingPtr, DVS_RX_BD_SPACE_BASE,
				DVS_RX_BD_SPACE_BASE,
	...
	for (Index = 0; Index < DVS_BUFFER_NUM; Index++) {
		...
		Status = XAxiDma_BdSetLength(BdCurPtr, DVS_BD_LEN,
				RxRingPtr->MaxTransferLen);
		if (Status != XST_SUCCESS) {
			xil_printf("Rx set length %d on BD %x failed %d\r\n",
					DVS_BD_LEN, (UINTPTR) BdCurPtr, Status);
			return XST_FAILURE;
		}
		...
		RxBufferPtr += DVS_BD_LEN;
		BdCurPtr = (XAxiDma_Bd *)XAxiDma_BdRingNext(RxRingPtr, BdCurPtr);
	}
	/*
	 * Set the coalescing threshold
	 */
	Status = XAxiDma_BdRingSetCoalesce(RxRingPtr, COALESCING_COUNT, //IRQ
			DELAY_TIMER_COUNT);
	if (Status != XST_SUCCESS) {
...
int config_dvs_cap_path(){
	...
	Dma_Config = XAxiDma_LookupConfig(XPAR_DVS_STREAM_DMA_DVS_DEVICE_ID);
	if (!Dma_Config) {
		xil_printf("No config found for %d\r\n", XPAR_DVS_STREAM_DMA_DVS_DEVICE_ID);
		return XST_FAILURE;
	}
