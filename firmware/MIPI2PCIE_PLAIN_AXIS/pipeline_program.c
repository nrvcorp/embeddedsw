...
#define XDPHY_DEVICE_ID		XPAR_MIPI_WRAPPER_MIPI_DPHY_0_DEVICE_ID
//XCsiSs CsiRxSs;
XDphy DphyRx;
...
int StartDVSSensor(void) {
	...
	XGpio_SetDataDirection(&DVS_rstn_Gpio, DVS_CHANNEL, ~DVS_RSTN);
...
//	}
	...
	for (int i= 0; i < sizeof(dphy_offset_list)/ sizeof(dphy_offset_list[0]); i++){
		u32 value = XDphy_ReadReg(XPAR_MIPI_WRAPPER_MIPI_DPHY_0_BASEADDR, dphy_offset_list[i]);
