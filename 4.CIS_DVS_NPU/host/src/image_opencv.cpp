...
extern "C"
{
    // Function to convert single 2-bit image to 8-bit data
    ...
    // function to count events per column and row
    // width, height : size of DVS window
    //  int* x_count : event count array for each column
    //  int* y_count : event count array for each row
    //  retval : total num of events
    ...
    // function to calculate approximate "square" roi from x_count and y_count, with size larger than roi_min_size
    //  width, height : size of DVS window
    //  int* x_count : event count array for each column
    //  int* y_count : event count array for each row
    //  int sum : input, total count of events
    //  int threshold : required consecutive number of columns or rows with events more than average to determine ROI window boundary
    //  int roi_min_size : lower bound to make ROI larger than NPU input image size
    //  float inflation_ratio : ratio to inflate the ROI window, since NPU detects smaller objects better than very large ones
    //  bbox* bounding_box : output, holds ROI window spatial information
    //  retval : whether if
    ...
    {
        // calculate x-axis ROI boundary from x_count
        int x_avg = sum / width;
        ...
        // calculate y-axis ROI boundary from y_count
        ...
        // calculate and inflate ROI window size
        ...
        // if inflated ROI window goes beyond frame boundary, move it to inside the frame
        ...
        // return if any ROI boundaries are detected
        // if not, the caller can refrain from updating bounding box
    ...
    */
    ...
    extern "C" void show_image_cv(image p, const char *name)
    {
        try
        {
            ...
            {
                cerr << "OpenCV exception in image_to_mat: " << e.what() << endl;
                free_image(copy);
                return;
            }
            ...
            {
                cerr << "OpenCV exception in cvtColor: " << e.what() << endl;
                free_image(copy);
                return;
            }
            ...
            {
                cerr << "OpenCV exception in namedWindow: " << e.what() << endl;
                free_image(copy);
                return;
            }
            ...
            {
                cerr << "OpenCV exception in imshow: " << e.what() << endl;
                free_image(copy);
                return;
            }
        ...
        {
            cerr << "Unknown exception in show_image_cv \n";
        }
    }
    ...
    */
    ...
    extern "C" image get_image_from_stream_resize(cap_cv *cap, int w, int h, int c, mat_cv **in_img, int dont_close)
    {
        ...
        // *(cv::Mat **)in_img = src;   // was not initially removed
        ...
        // release_capture(cap); // added
        release_mat((mat_cv **)&src); // added
