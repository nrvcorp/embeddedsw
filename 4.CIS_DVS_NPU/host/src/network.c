...
int max_abs_int(float src, int max_val)
{
    if (abs(src) > abs(max_val))
        src = (src > 0) ? max_val : -max_val - 1;
    else
        src = round_int(src);
    return (int)src;
}

char *file_read_wrapper(char *filename, size_t total_bytes)
{
    char *out_buffer = (char *)malloc(total_bytes);
    int fd = open(filename, O_RDONLY);
    read_to_buffer(filename, fd, out_buffer, total_bytes, 0);
    return out_buffer;
}

void file_write_wrapper(char *buffer, char *filename, size_t total_bytes)
{
    int fd = open(filename, O_RDWR | O_CREAT | O_TRUNC | O_SYNC, 0666);
    write_from_buffer(filename, fd, buffer, total_bytes, 0);
}

int buffer_compare(char *buffer, char *buffer_ref, size_t max_idx)
{
    printf("buffer compare: \n");
    for (int idx = 0; idx < max_idx; idx++)
    {
        if (buffer_ref[idx] != buffer[idx])
        {
            printf("index %d mismatch, ref: 0x%02x, result: 0x%02x\n", idx, buffer_ref[idx] & 0xFF, buffer[idx] & 0xFF);
            return 1;
        }
    }
    return 0;
...
void forward_network(network net, network_state state)
{
    ...
    // printf("starting forward network \r\n");

    // 1. state.input: image tensor (data to send to PCIE)
    // get image ch, w, h
    ...
    // FILE *fp = fopen("../DATASET/TINY_YOLOv3/ifm_00.hex", "r");
    // if(fp==NULL){
    ...
    //             sscanf(line, "%d",&number ); ///!!!! hex file contained decimal numbers, need to change for hexadecimal
    //             if(number != (unsigned char)(in_buffer[src])){
    //                 printf("error at line %d %d %d, values %u %d\n",c_idx, h_idx, w_idx, (unsigned char)in_buffer[src],number);
    //             }
    //         }
    //     }
    // }
    ...
    ////---------------write contents of in_buffer ( = ifm_case 0 formatted image) to file--------------
    // char* outfname = "../DATASET/TINY_YOLOv3/store_ifm00.bin";
    // file_write_wrapper(in_buffer, outfname, in_bytes);
    ////------------------------------------------------------------------------------------------------

    ////---------------obtain NPU input from  file instead of image --------------------------
    // char* infname = "../DATASET/TINY_YOLOv3/ifm00.bin";
    ...
    for (i = 0; i < net.n; ++i)
    {
        state.index = i;
        layer l = net.layers[i];
        layer prev_l = net.layers[prev_i];

        //------------------------------------
        // 1. Send Layer configuration to FPGA
        // 2. Run Layer
        //------------------------------------
        if (l.type == CONVOLUTIONAL)
        {
            ...
            if (i != 0)
            {
                ...
                // int npu_out_w;
                // int npu_out_h;
                // if(prev_l.layer_npu.ifm_is_maxpool && prev_l.layer_npu.pool_stride == 2){
                //     npu_out_w = (l.w+3)/4;
                //     npu_out_h = l.h;
                // }else{
                //     npu_out_w = (l.w+1)/2;
                ...
                // }
                // int npu_out_c;
                // int npu_out_c_concat;
                // if(l.layer_npu.is_concat){
                //     npu_out_c = (l.layer_npu.in_ch - l.layer_npu.concat_ch + 7)/ 8;
                //     npu_out_c_concat = (l.layer_npu.concat_ch + 7)/ 8;
                // }else{
                //     npu_out_c = (l.c+7)/8;
                // }
                ...
                // if(i==18){
                //     read_to_buffer(net.c2h_device, net.c2h_fd, out_buffer,
                //         out_bytes, YOLOv3_IFM_BUFFER_2);
                // }else if(l.layer_npu.is_concat){
                //     read_to_buffer(net.c2h_device, net.c2h_fd, out_buffer,
                //         out_bytes, YOLOv3_IFM_BUFFER_1);
                //     int out_bytes_concat =  npu_out_w * npu_out_h * 4 * npu_out_c_concat * 8;
                //     char * out_buffer_concat = (char*) malloc(out_bytes_concat);
                //     read_to_buffer(net.c2h_device, net.c2h_fd, out_buffer_concat,
                //         out_bytes_concat, l.layer_npu.concat_baseaddr);
                //     char * outref_concat_fname;
                //     asprintf(&outref_concat_fname, "../DATASET/TINY_YOLOv3/CONV%02d/ifm1_%02d.bin",i, i);
                //     char *out_buffer_concat_ref = file_read_wrapper(outref_concat_fname,out_bytes_concat);
                //     printf("\r\n\r\nlayer : %d\r\n\r\n", i);
                //     buffer_compare(out_buffer_concat_ref, out_buffer_concat, out_bytes_concat);
                //     free(outref_concat_fname);
                //     free(out_buffer_concat_ref);
                //     free(out_buffer_concat);
                // }else{
                //     read_to_buffer(net.c2h_device, net.c2h_fd, out_buffer,
                //         out_bytes, prev_l.layer_npu.ofm_baseaddr);
                // }
                // char * outref_fname;
                // asprintf(&outref_fname, "../DATASET/TINY_YOLOv3/CONV%02d/ifm%02d.bin",i, i);
                // char* out_buffer_ref = file_read_wrapper(outref_fname, out_bytes);
                // printf("\r\n\r\nlayer : %d\r\n\r\n", i);
                // buffer_compare(out_buffer_ref, out_buffer, out_bytes);
                // free(outref_fname);
                // free(out_buffer_ref);
                // free(out_buffer);
                // }
        ...
        // 2. convert data case, dtype to float32
        // 3. do yolo forward
        //-----------------------------------
        if (l.type == YOLO)
        {
            ...
            int npu_out_w = (l.w + 1) / 2;
            int npu_out_h = (l.h + 1) / 2;
            int npu_out_c = (l.c + 7) / 8;
            int out_bytes = npu_out_w * npu_out_h * 4 * npu_out_c * 8;
            char *out_buffer = (char *)malloc(out_bytes);
            // 1. Read from FPGA to state.input
            uint32_t status = *((uint32_t *)(net.user_base + AXILITE_STATUS));
            ...
            // TODO : create pthread to execute PCIE and NPU in parallel
            read_to_buffer(net.c2h_device, net.c2h_fd, out_buffer,
                           out_bytes, prev_l.layer_npu.ofm_baseaddr);

            //--------------------verify ifm_case 1 formatted output of prev layer---------------------------
            // printf("verifying layer %d:\n", i);
            // char * outref_fname;
            // if(l.w == 13){
            //     asprintf(&outref_fname, "../DATASET/TINY_YOLOv3/CONV15/ofm15.bin");
            // }else{
            //     asprintf(&outref_fname, "../DATASET/TINY_YOLOv3/CONV22/ofm22.bin");
            // }
            // char* out_buffer_ref = file_read_wrapper(outref_fname, out_bytes);
            // buffer_compare(out_buffer_ref, out_buffer, out_bytes);
            // free(outref_fname);
            ...
            // char * outref_fname;
            // if(l.w == 13){
            //     asprintf(&outref_fname, "../DATASET/TINY_YOLOv3/CONV15/ofm_not_format15.bin");
            // }else{
            //     asprintf(&outref_fname, "../DATASET/TINY_YOLOv3/CONV22/ofm_not_format22.bin");
            // }
            // char* out_buffer_ref = file_read_wrapper(outref_fname,l.w*l.h*l.c);
            // free(outref_fname);
            //-----------------------------------------
            for (int c_idx = 0; c_idx < l.c; c_idx++)
            {
                for (int h_idx = 0; h_idx < l.h; h_idx++)
                {
                    for (int w_idx = 0; w_idx < l.w; w_idx++)
                    {
                        int c_grp = c_idx >> 3;
                        int h_grp = h_idx >> 1;
                        int w_grp = w_idx >> 1;
                        int dest = c_idx * l.w * l.h + h_idx * l.w + w_idx;
                        int src_base = 32 * (w_grp + c_grp * npu_out_w + h_grp * npu_out_w * npu_out_c);
                        int src_offset = (c_idx & 0x7) + 8 * (w_idx & 0x1) + 16 * (h_idx & 0x1);
                        int result = out_buffer[src_base + src_offset];
                        state.input[dest] = (float)(result)*yolo_scale + yolo_bias;
                        ////------------------for debug, compare with 22 layer ofm without any reordering-----------
                        // if ((int8_t)(out_buffer_ref[dest]) != result) {
                        //     printf("layer: %d :: index %d %d %d dest : %d src : %d mismatch, ref: 0x%02x, result: 0x%02x\n", i, c_idx, h_idx, w_idx, dest, src_base+src_offset, (int8_t)(out_buffer_ref[dest]) & 0xFF, result & 0xFF);
                        // }
                        ////----------------------------------------------------------------------------------------
                    }
                }
            ...
            // 3. do yolo forward
            l.forward(l, state);
            // TODO : display YOLO output for both yolo layers
        }
