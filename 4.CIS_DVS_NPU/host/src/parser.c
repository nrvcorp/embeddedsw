...
{
    //--------------------------------------------------
    // 1. Configure PCIE device
    // 2. Connect, Check PCIE
    //--------------------------------------------------

    net->h2c_device = H2C_DEVICE;
    net->c2h_device = C2H_DEVICE;
    net->reg_device = REG_DEVICE;
    net->user_device = USER_DEVICE;

    printf(ANSI_COLOR_YELLOW);
    printf("************************************************\r\n");
    printf("*******  NPU PCIE configuration      ***********\r\n");
    printf("************************************************\r\n");
    //--------------------------------------------------
    // Configure NPU PCIE device
    //--------------------------------------------------

    /* check h2c functionality */
    net->h2c_fd = open(net->h2c_device, O_RDWR);
    if (net->h2c_fd < 0)
    {
        fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->h2c_device, net->h2c_fd);
        perror("open device");
        return;
    }
    else
    {
        printf(ANSI_COLOR_YELLOW "> h2c_device: " ANSI_COLOR_RESET);
        printf("%s\r\n", net->h2c_device);
    }
    /* check c2h functionality */
    net->c2h_fd = open(net->c2h_device, O_RDWR);
    if (net->c2h_fd < 0)
    {
        fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->c2h_device, net->c2h_fd);
        perror("open device");
        return;
    }
    else
    {
        printf(ANSI_COLOR_YELLOW "> c2h_device: " ANSI_COLOR_RESET);
        printf("%s\r\n", net->c2h_device);
    }

    net->reg_fd = open(net->reg_device, O_RDWR);
    if (net->reg_fd < 0)
    {
        fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\n" ANSI_COLOR_RESET,
                net->reg_device, net->reg_fd);
        perror("open reg device failed");
    }
    else
    {
        printf(ANSI_COLOR_YELLOW "> reg_device: " ANSI_COLOR_RESET);
        printf("%s\r\n", net->reg_device);
    }

    net->user_fd = open(net->user_device, O_RDWR | O_SYNC);
    if (net->user_fd < 0)
    {
        fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\n" ANSI_COLOR_RESET,
                net->user_device, net->user_fd);
        perror("open USER device failed");
    }
    else
    {
        printf(ANSI_COLOR_YELLOW "> user_device: " ANSI_COLOR_RESET);
        printf("%s\r\n", net->user_device);
    }

    net->user_base = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, net->user_fd, 0);
    if (net->user_base == (void *)-1)
    {
        printf("error unmap \r\n");
    }
    //--------------------------------------------------
    // Configure DVS PCIE device
    //--------------------------------------------------

    // net->dvs_h2c_device = DVS_H2C_DEVICE;
    // net->dvs_c2h_device = DVS_C2H_DEVICE;
    // /* check h2c functionality */
    // net->dvs_h2c_fd = open(net->dvs_h2c_device, O_RDWR);
    // if (net->dvs_h2c_fd < 0)
    // {
    //     fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->dvs_h2c_device, net->dvs_h2c_fd);
    //     perror("open device");
    //     return;
    // }
    // else
    // {
    //     printf(ANSI_COLOR_YELLOW "> dvs_h2c_device: " ANSI_COLOR_RESET);
    //     printf("%s\r\n", net->dvs_h2c_device);
    // }
    // /* check c2h functionality */
    // net->dvs_c2h_fd = open(net->dvs_c2h_device, O_RDWR);
    // if (net->dvs_c2h_fd < 0)
    // {
    //     fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->dvs_c2h_device, net->dvs_c2h_fd);
    //     perror("open device");
    //     return;
    // }
    // else
    // {
    //     printf(ANSI_COLOR_YELLOW "> dvs_c2h_device: " ANSI_COLOR_RESET);
    //     printf("%s\r\n", net->dvs_c2h_device);
    // }
    //--------------------------------------------------
    // Configure CIS PCIE device
    //--------------------------------------------------

    // net->cis_h2c_device = CIS_H2C_DEVICE;
    // net->cis_c2h_device = CIS_C2H_DEVICE;
    // /* check h2c functionality */
    // net->cis_h2c_fd = open(net->cis_h2c_device, O_RDWR);
    // if (net->cis_h2c_fd < 0)
    // {
    //     fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->cis_h2c_device, net->cis_h2c_fd);
    //     perror("open device");
    //     return;
    // }
    // else
    // {
    //     printf(ANSI_COLOR_YELLOW "> cis_h2c_device: " ANSI_COLOR_RESET);
    //     printf("%s\r\n", net->cis_h2c_device);
    // }
    // /* check c2h functionality */
    // net->cis_c2h_fd = open(net->cis_c2h_device, O_RDWR);
    // if (net->cis_c2h_fd < 0)
    // {
    //     fprintf(stderr, ANSI_COLOR_RED "unable to open device %s, %d.\r\n" ANSI_COLOR_RESET, net->cis_c2h_device, net->cis_c2h_fd);
    //     perror("open device");
    //     return;
    // }
    // else
    // {
    //     printf(ANSI_COLOR_YELLOW "> cis_c2h_device: " ANSI_COLOR_RESET);
    //     printf("%s\r\n", net->cis_c2h_device);
    // }
    ...
    char *wgt_buffer = (char *)malloc(0x10000000); // set as max size
    size_t yolo_wgt_addr = YOLOv3_WGT_BASEADDR;
    for (int l = 0; l < net->n; l++)
    {
        if (net->layers[l].type == CONVOLUTIONAL)
        {
            int conv_mode = net->layers[l].layer_npu.conv_mode;
            int in_width = net->layers[l].layer_npu.in_width;
            int in_height = net->layers[l].layer_npu.in_height;
            int in_ch = net->layers[l].layer_npu.in_ch;
            int out_ch = net->layers[l].layer_npu.out_ch;
            int ifm_case = net->layers[l].layer_npu.ifm_case;
            int is_padding = net->layers[l].layer_npu.ifm_is_padding;
            int is_maxpool = net->layers[l].layer_npu.ifm_is_maxpool;
            int pool_stride = net->layers[l].layer_npu.pool_stride;
            int is_concat = net->layers[l].layer_npu.is_concat;
            int concat_ch = net->layers[l].layer_npu.concat_ch;
            int is_upsample = net->layers[l].layer_npu.is_upsample;
            int single_bias = net->layers[l].layer_npu.is_single_scale_bias;

            int kernel_size = (conv_mode == 1) ? 3 : 1;
            int in_channel_pkt = (int)((in_ch + 7) / 8);
            int out_channel_pkt = (int)((out_ch + 7) / 8);

            int wgt_bytes;
            wgt_bytes = out_channel_pkt * 32 + out_channel_pkt * in_channel_pkt * kernel_size * kernel_size * 8 * 8;
            printf("while writing, bias_baseaddr of layer %d was %lx\r\n\r\n", l, yolo_wgt_addr);
            net->layers[l].layer_npu.bias_baseaddr = yolo_wgt_addr;
            net->layers[l].layer_npu.weight_baseaddr = yolo_wgt_addr + 32 * out_channel_pkt;
            yolo_wgt_addr += wgt_bytes;
            ...
            if (wgtfile_fd < 0)
            {
                fprintf(stderr, ANSI_COLOR_RED "unable to open weight file %s, %d.\n" ANSI_COLOR_RESET,
                        wgt_filename, wgtfile_fd);
            }
            read_to_buffer(wgt_filename, wgtfile_fd, wgt_buffer, wgt_bytes, 0);
            write_from_buffer(net->h2c_device, net->h2c_fd, wgt_buffer,
                              wgt_bytes, net->layers[l].layer_npu.bias_baseaddr);
            free(wgt_filename);
            printf("Current Weight Data address : %lx, wgt_bytes: %d, out_ch: %d \r\n\r\n",
                   yolo_wgt_addr, wgt_bytes, out_ch);
        }
        else if (net->layers[l].type == ROUTE)
        {
            if (net->layers[l].n == 2)
            {
                int prev_l = net->layers[l].input_layers[1];
                net->layers[l].layer_npu.bias_baseaddr = net->layers[prev_l].layer_npu.bias_baseaddr;
                net->layers[l].layer_npu.weight_baseaddr = net->layers[prev_l].layer_npu.weight_baseaddr;
            }
        }
    }
    printf("Upper Bound of Weight Data Area : %lx\r\n\r\n", yolo_wgt_addr);
    free(wgt_buffer);
}
