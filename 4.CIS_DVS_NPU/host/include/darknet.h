...
typedef struct
{
	...
	uint32_t ifm_case;
	uint32_t ifm_dtype;
	uint32_t ifm_is_padding;
	uint32_t ifm_is_maxpool;
	uint32_t ifm_is_leaky_relu;
	uint32_t is_concat;
	uint32_t is_upsample;
	uint64_t ifm_baseaddr;
	uint64_t weight_baseaddr;
	uint64_t ofm_baseaddr;
	uint32_t in_width;
	uint32_t in_height;
	uint32_t in_ch;
	uint32_t out_ch;
	...
	uint32_t bias_shift;
	uint32_t is_single_scale_bias;
	uint32_t single_scale;
	uint32_t single_bias;
	uint64_t concat_baseaddr;
	uint32_t concat_ch;
	uint64_t bias_baseaddr;
} NPU_LayerConfig;
...
// LIB_API void camera_demo(char *datacfg, char *cfgfile, char *weightfile, float thresh, float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers);
