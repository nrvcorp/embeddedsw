...
void handleMode(Mode mode)
{
    ...
    case DVS_DISPLAY:
        ...
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager);
        dvs->display_stream(true); // Call DVS display stream
        delete dvs;                // Cleanup
        dvs = NULL;
        break;

    case DUAL_DVS_DISPLAY:
        printf("Dual DVS display mode \n");
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR,
            DVS_BUFFER_NUM, C2H_DEVICE_DVS, H2C_DEVICE_DVS,
            mutexManager);
        dvs1 = new DVS(
            1, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR_1, DVS_FRAME_BASEADDR_1,
            DVS_BUFFER_NUM, C2H_DEVICE_DVS_1, H2C_DEVICE_DVS_1,
            mutexManager);
        ...
        if (dvs1)
        {
            threads.emplace_back([dvs1]()
                                 { dvs1->display_stream(true); });
        }
    ...
    case DVS_CHECK_FRAME_DROP:
        ...
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, 1,
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager);
    ...
    case CIS_DVS_DISPLAY:
        ...
        cis = new CIS(
            CIS_FRAME_H, CIS_FRAME_W, CIS_FRAME_RDY_BASEADDR,
            CIS_FRAME_BASEADDR, CIS_BUFFER_NUM, C2H_DEVICE_CIS,
            H2C_DEVICE_CIS, mutexManager);
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager);
    ...
    case DVS_STORE:
        ...
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, 1,
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager, true);
        // Start threads for CIS and DVS
        if (dvs)
        {
            threads.emplace_back([dvs]()
    ...
    case DVS_ROI:
        ...
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager, &bbox,
            &bbox_mutex, &terminate);
        dvs->set_DVS_ROI(
            ROI_EVENT_SCORE, ROI_MIN_SCORE, ROI_LINE_WIDTH,
            DVS_ROI_MIN_SIZE, 1.0);
    ...
    case CIS_DVS_BBOX:
        ...
        cis = new CIS(
            CIS_FRAME_H, CIS_FRAME_W, CIS_FRAME_RDY_BASEADDR,
            CIS_FRAME_BASEADDR, CIS_BUFFER_NUM, C2H_DEVICE_CIS,
            H2C_DEVICE_CIS, mutexManager, &bbox_mutex, &bbox,
            &terminate);
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true, (DVS_FPS / DISPLAY_FPS),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
        ...
        cis->set_DVS(
            CIS_DVS_SCALE_X * CIS_FRAME_W / DVS_FRAME_W,
            CIS_DVS_SCALE_Y * CIS_FRAME_H / DVS_FRAME_H,
            CIS_DVS_OFFSET_X, CIS_DVS_OFFSET_Y);
        dvs->set_CIS(
            CIS_DVS_SCALE_X * CIS_FRAME_W / DVS_FRAME_W,
            CIS_DVS_SCALE_Y * CIS_FRAME_H / DVS_FRAME_H,
            CIS_DVS_OFFSET_X, CIS_DVS_OFFSET_Y, CIS_FRAME_W,
            CIS_FRAME_H, ROI_EVENT_SCORE, ROI_MIN_SCORE,
            ROI_LINE_WIDTH, CIS_ROI_MIN_SIZE, ROI_INFLATION);
        // Start threads for CIS and DVS
        if (cis)
        {
            threads.emplace_back([cis]()
    ...
    case CIS_DVS_DISPLAY_FPS:
        ...
        cis = new CIS(
            ...
            H2C_DEVICE_CIS, mutexManager);
        dvs = new DVS(
            0, DVS_FRAME_H, DVS_FRAME_W, true,
            (DVS_FPS / (DISPLAY_FPS * DISPLAY_DOWNSAMPLE_NUM)),
            DVS_FRAME_RDY_BASEADDR, DVS_FRAME_BASEADDR, DVS_BUFFER_NUM,
            C2H_DEVICE_DVS, H2C_DEVICE_DVS, mutexManager, true,
            DISPLAY_DOWNSAMPLE_NUM);
        // Start threads for CIS and DVS
        if (cis)
        {
            threads.emplace_back([cis]()
...
Mode parseArguments(int argc, char *argv[])
{
    ...
    struct option long_options[] = {
        ...
        {"dual-dvs", no_argument, nullptr, 'D'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "cdDxswrbofpi", long_options, nullptr)) != -1)
    {
        switch (opt)
        {
        case 'c':
