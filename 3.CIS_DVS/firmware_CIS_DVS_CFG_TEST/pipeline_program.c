...
// This section has been moved to pipeline_program.h
/*
...
volatile u64 dvs_frame_array[DVS_BUFFER_NUM];
volatile u64 dvs_frame_rdy[DVS_BUFFER_NUM];
...
extern volatile u64 fil_frame_rdy[FIL_BUFFER_NUM];
extern volatile u64 fil_frame_array[FIL_BUFFER_NUM];
extern u32 g_dma_fil_rd_work_ptr;
extern u32 NumOfEmptyDVSBuf;
#endif
...
u8 frame_drop_check_init=1;
int frame_drop_count_dvs = 0;
...
void configure_buffer_system() {
	...
	for (int i=0; i < FIL_BUFFER_NUM; ++i) {
		fil_frame_array[i] = fil_frame_addr;
		fil_frame_rdy[i] = fil_frame_rdy_addr;
		fil_frame_addr += FIL_BUF_SIZE;
		fil_frame_rdy_addr += 1;
	}
	memset((u64 *)fil_frame_rdy[0], 0b0, FIL_BUFFER_NUM);
	Xil_DCacheInvalidateRange((UINTPTR)FIL_BUF_BASE_ADDR, FIL_BUFFER_NUM*FIL_BUF_SIZE);
	Xil_DCacheInvalidateRange((UINTPTR)FIL_BUFFER_RDY, FIL_BUFFER_NUM);
#endif
...
//	xil_printf("DVS DMA Wr Done %d\r\n", dvs_frm_cnt);
...
#if(CHECK_DVS_ISR_INTERVAL)
	IntervalRecordStart(INTERVAL_DVS_DMA_DONE_ISR);
#endif
#if(ENABLE_HOST_DIRECT_DVS_ACCESS)
	#if(CHECK_DVS_BUFFER_HOST_DRAIN)
		if(Xil_In8(dvs_frame_rdy[ dvs_wr_ptr % DVS_BUFFER_NUM ]) == 0x1){
			DEBUG_PRINT(DEBUG, "Host is too slow to retrieve DVS data via PCIe in time.\r\n");
			host_delay_count_dvs++;
		}
	#endif

	//if(dvs_wr_ptr / DVS_BUFFER_NUM == 0) Xil_Out8(dvs_frame_rdy[ dvs_wr_ptr % DVS_BUFFER_NUM ], 0b1);
	//else Xil_Out8(dvs_frame_rdy[ dvs_wr_ptr % DVS_BUFFER_NUM ], 0b11);
	u32 wptr = dvs_wr_ptr % DVS_BUFFER_NUM;
	...
	u64 h;
	u32 fnum, tstmp;
	static u32 prev_fnum=0, prev_tstmp=0;

	h = Xil_In64(dvs_frame_array[dvs_wr_ptr % DVS_BUFFER_NUM]);
	fnum = (h >> 32) & 0xFFFFFFFF;
	tstmp = h & 0xFFFFFFFF;

	if(frame_drop_check_init==0){
		if(fnum-prev_fnum != 1 && prev_fnum-fnum != 255)
		{
	...
	prev_fnum = fnum;
	prev_tstmp = tstmp;
#endif // CHECK_DVS_FRAME_DROP


	if (dvs_wr_ptr == DVS_BUFFER_NUM*2 -1) {
		dvs_wr_ptr = 0;
	} else {
...
int DVSReadData(u16 ByteCount){
	...
	/* Send the data */
	XIicPs_MasterRecv(&IicPsInstance, DVSIICReadBuf,ByteCount, DVS_ADDR);
	/* Wait till the transmission is completed */
	while (DVSIICReceiveComplete == FALSE) {
		if (0 != DVSIICTotalErrorCount) {
			return XST_FAILURE;
...
int ProgramDVSSensor(struct regval_list DVS_reg_cfg[], const int length) {
	...
	u32 MaxIndex = length;
	DVSIicAddr = DVS_ADDR;
	struct regval_list * sensor_cfg = DVS_reg_cfg;
...
int dvs_dma_reset(){
	...
	if(XAxiDma_ResetIsDone(&DVSDma)==0) return XST_FAILURE;
