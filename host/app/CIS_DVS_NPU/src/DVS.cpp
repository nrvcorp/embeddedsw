...
DVS::DVS( //normal constructor
    ...
    MutexManager &display_mutex, int double_buffering)
...
// DVS::DVS( // double buffer constructor
//     int frame_h, int frame_w,
//     bool is_header, int accum_num,
//     uintptr_t rdy_baseaddr, uintptr_t frame_baseaddr,
//     int buffer_num,
...
DVS::DVS( // crop_roi constructor
    ...
    float roi_inflation_ratio, Bbox *bbox, MutexManager *thread_mutex,
    bool* terminate)
    : frame_h(frame_h), frame_w(frame_w), accum_num(accum_num),
      is_header(is_header), header_bytes(8),
      rdy_baseaddr(rdy_baseaddr), frame_baseaddr(frame_baseaddr),
...
void DVS::crop_coord(int img_show = 1)
{
    while (true)
    {
        frame = cv::Mat::zeros(frame_h, frame_w, CV_8UC1);
        ...
        for (int frame_grp_num = 0; frame_grp_num < accum_num; frame_grp_num++)
        {
            ...
            sum += event_accum(x_count, y_count, frame_w, frame_h);
        }
        Bbox b_box;
        ...
        thread_mutex->unlock_single_writer(is_roi);
        display_mutex.lock_display();
        if (img_show)
        {
...
int DVS::event_accum(int *x_count, int *y_count, int width, int height)
{
    int sum = 0;
    for (int h = 0; h < height; h++)
    {
        for (int byteIndex = 0; byteIndex < (width >> 2); ++byteIndex)
        {
            ...
            {
                uint8_t pixel = (frame_start[h * (width >> 2) + byteIndex] >> bitOffset) & 0x03; // Extract 2 bits
                if (pixel != 0)
                {
